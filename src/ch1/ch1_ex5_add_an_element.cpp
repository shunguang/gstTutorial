...
int ch1_ex5_add_an_element(int argc, char* argv[])
{
	...
	GstElement* videoSrc = NULL, *textOverlay=NULL, *videoSink = NULL;
	GstBus* bus = NULL;
	//GstMessage *msg = NULL;
  ...
  */
  ...
  if (!pipeline || !videoSrc || !textOverlay || !videoSink || !loop)
  {
  ...
  gst_bin_add_many(GST_BIN(pipeline), videoSrc, textOverlay,videoSink, NULL);
  suc = gst_element_link_many(videoSrc, textOverlay, videoSink, NULL);
  if (!suc)
  {

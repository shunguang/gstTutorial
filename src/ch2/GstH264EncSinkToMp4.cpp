#include "GstH264EncSinkToMp4.h"

using namespace std;
using namespace app;

GstH264EncSinkToMp4::GstH264EncSinkToMp4(const int w, const int h, const int fpsNum, const int fpsDen, const std::string &mp4FilePath)
	: m_imgW(w)
	, m_imgH(h)
	, m_fps(fpsNum, fpsDen)
...
GstH264EncSinkToMp4::~GstH264EncSinkToMp4()
{
	if (m_isInit) {
		endEnc();
	}
...
void GstH264EncSinkToMp4::init()
{
	...
	if (!app->pipeline ||
		...
		!app->sink) {
		appAssert(0, "GstH264EncSinkToMp4(): Error creating pipeline elements!");
	}
	...
	appAssert(state_ret == GST_STATE_CHANGE_ASYNC, " GstH264EncSinkToMp4::init(): cannot set state to paly()!" );
	m_isInit = true;
}


int GstH264EncSinkToMp4::addFrm(const HostYuvFrm *yuv)
{
	if ( !m_isInit ) {
		return -1;
	}
	...
	if (yuvImgBuf == NULL) {
		appAssert(false, "GstH264EncSinkToMp4::addFrm(): can not alocated memory!");
	}
   ...
   //copy image from gpu to cpu
	...
	if (ret != GST_FLOW_OK) {
		dumpLog("GstH264EncSinkToMp4::addFrm(): fn=%d, sth wrong!", yuv->fn_);
		return -1;
	}

	if ( yuv->fn_ % 500 == 0) {
		dumpLog("GstH264EncSinkToMp4::addFrm(): fn=%d", yuv->fn_);
	}
	//note: the memoery of <yuvImgBuf> will be freeed inside gst
	return 0;
}

int GstH264EncSinkToMp4::endEnc()
{
	gst_app_src_end_of_stream(GST_APP_SRC(m_gstApp.src));
	APP_SLEEP_MS(50);
	dumpLog("GstH264EncSinkToMp4::endEnc(): well done!");
	m_isInit = false;
	return 0;
}
